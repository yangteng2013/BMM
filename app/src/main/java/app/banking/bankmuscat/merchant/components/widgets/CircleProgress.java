...
public class CircleProgress extends View {
	...
	private final int default_finished_color = Color.rgb(211, 0, 43);
	...
	public CircleProgress(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		default_text_size = Utils.sp2px(getResources(), 20);
		min_size = (int) Utils.dp2px(getResources(), 100);

		final TypedArray attributes = context.getTheme()
				.obtainStyledAttributes(attrs, R.styleable.CircleProgress,
						defStyleAttr, 0);
	...
	protected void initByAttributes(TypedArray attributes) {
		...
		textColor = attributes.getColor(
				R.styleable.CircleProgress_circle_text_color,
				default_text_color);
		textSize = attributes.getDimension(
				R.styleable.CircleProgress_circle_text_size, default_text_size);

		setMax(attributes.getInt(R.styleable.CircleProgress_circle_max,
				default_max));
		setProgress(attributes.getInt(
				R.styleable.CircleProgress_circle_progress, 0));

		if (attributes.getString(R.styleable.CircleProgress_circle_prefix_text) != null) {
			setPrefixText(attributes
					.getString(R.styleable.CircleProgress_circle_prefix_text));
		}
		if (attributes.getString(R.styleable.CircleProgress_circle_suffix_text) != null) {
			setSuffixText(attributes
					.getString(R.styleable.CircleProgress_circle_suffix_text));
		}
	}
	...
	public String getDrawText() {
		return getSuffixText() + getPrefixText();
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		rectF.set(0, 0, MeasureSpec.getSize(widthMeasureSpec),
	...
	protected void onDraw(Canvas canvas) {
		...
		if (!TextUtils.isEmpty(text)) {
			canvas.drawText(text,
					...
					((int) ((canvas.getHeight() / 2) - ((textPaint.descent() + textPaint.ascent()) / 2))) + 125, textPaint);
		}
	}
